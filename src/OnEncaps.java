        ...
        for (int thetaSize = 3; thetaSize <= 30; thetaSize += 3) {
            ...
            for (int l = 10; l <= 100; l += 10) {
                ...
                for (int repeat = 0; repeat < repeatCount; repeat++) {
                    ...
                    long dcBytes = C1.toBytes().length;
                    for (int i = 1; i <= l; i++) {
                        for (int sig = 1; sig <= thetaSize; sig++) {
                            dcBytes += C_i_sig_2_1[i - 1][sig - 1]
                                    .toBytes().length;
                        }

                        dcBytes += C_i_2_2[i - 1].toBytes().length;
                        dcBytes += C_i_3[i - 1].toBytes().length;

                        for (int sig = 1; sig <= thetaSize; sig++) {
                            dcBytes += C_i_sig_4[i - 1][sig - 1]
                                    .toBytes().length;
                        }

                        dcBytes += C_i_5_1[i - 1].toBytes().length;
                        dcBytes += C_i_5_2[i - 1].toBytes().length;
                    }

                    totalDCBytes += dcBytes;


                    if (C_i_5_3.length != l || C_i_5_3[l - 1] == null) {
                        throw new IllegalStateException("C_i_5(3) was not generated.");
                    }
                }

                double averageTimeMs = totalTimeMs / repeatCount;
                double averageDCKBytes = (double) totalDCBytes
                        / repeatCount
                        / 1024.0;

                System.out.printf("Theta=%d, l=%d%n", thetaSize, l);
                System.out.printf(
                        "OnEncaps computation time: %.3f ms%n",
                        averageTimeMs
                );
                System.out.printf(
                        "DC storage: %.3f KBytes%n",
                        averageDCKBytes
                );
                System.out.println("--------------------------------------------------");
            }
