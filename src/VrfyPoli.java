    ...
    private static void runForCurve(String curveFile) {
        ...
        for (int thetaSize = 3; thetaSize <= 30; thetaSize += 3) {
            ...
            for (int l = 10; l <= 100; l += 10) {
                ...
                double totalVrfyPoliTimeMs = 0.0;
                ...
                for (int repeat = 0; repeat < repeatCount; repeat++) {
                    ...
                    Element ck1 = G0.newRandomElement().getImmutable();
                    Element ck2 = G1.newRandomElement().getImmutable();


                    Element[] dpr = new Element[l];
                    Element[] temp_i = new Element[l];
                    ...
                    for (int i = 1; i <= l; i++) {
                        dpr[i - 1] = Zp.newRandomElement().getImmutable();
                    }
                    ...
                    for (int i = 1; i <= l; i++) {
                        Element gidPairing = pairing
                                .pairing(H_gid, Cpr_i_1_4[i - 1])
                                .getImmutable();
                        Element numerator = pf[i - 1]
                                .duplicate()
                                .mul(gidPairing)
                                .getImmutable();

                        Element ck2Tpr_i = ck2
                                .powZn(tpr[i - 1])
                                .getImmutable();
                        Element denominator = pairing
                                .pairing(ck1, ck2Tpr_i)
                                .getImmutable();

                        temp_i[i - 1] = numerator
                                .duplicate()
                                .div(denominator)
                                .getImmutable();
                    }
                    ...
                    for (int i = 1; i <= l; i++) {
                        Element weightedTemp_i = temp_i[i - 1]
                                .powZn(dpr[i - 1])
                                .getImmutable();
                        temp.mul(weightedTemp_i);
                    }
                    temp = temp.getImmutable();


                    Element expectedValue = pairingBase
                            .powZn(v)
                            .getImmutable();
                    boolean verificationResult = temp.isEqual(expectedValue);

                    long endTime = System.nanoTime();
                    totalVrfyPoliTimeMs += (endTime - startTime)
                            / 1_000_000.0;


                    benchmarkSink = new Object[] {
                            temp_i, temp, expectedValue,
                            verificationResult, pf, Cpr_i_1_4,
                            gamma, ck1, ck2, C1, beta
                    };
                    ...
                    if (temp_i[l - 1] == null
                        ...
                        throw new IllegalStateException(
                                "Policy verification failed to execute."
                        );
                    }
                }

                double averageVrfyPoliTimeMs = totalVrfyPoliTimeMs
                        / repeatCount;
                ...
                System.out.printf(
                        "VrfyPoli Computation Time: %.3f ms%n",
                        averageVrfyPoliTimeMs
                );
                System.out.println("--------------------------------------------------");
            }
