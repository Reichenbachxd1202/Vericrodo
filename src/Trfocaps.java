        ...
        for (int thetaSize = 3; thetaSize <= 30; thetaSize += 3) {
            ...
            for (int l = 10; l <= 100; l += 10) {
                ...
                for (int repeat = 0; repeat < repeatCount; repeat++) {
                    ...
                    long tcBytes = C1.toBytes().length;
                    for (int i = 1; i <= l; i++) {
                        tcBytes += T[i - 1].toBytes().length;
                        tcBytes += C_i_sig_4[i - 1][0].toBytes().length;
                    }
                    totalTCBytes += tcBytes;
                }

                double averageTimeMs = totalTimeMs / repeatCount;
                double averageTCKBytes = (double) totalTCBytes
                        / repeatCount
                        / 1024.0;

                System.out.printf("Theta=%d, l=%d%n", thetaSize, l);
                System.out.printf(
                        "Trfocaps computation time: %.3f ms%n",
                        averageTimeMs
                );
                System.out.printf(
                        "TC storage: %.3f KBytes%n",
                        averageTCKBytes
                );
                System.out.println("--------------------------------------------------");
